...
urlpatterns = [
    ...
    # Workflow API
    #
    # We don't use Django's 'end-URL-with-a-slash' convention here, because
    # it's unique to Django. (Really, we oughtn't to use it _anywhere_, but the
    # API is a good place to start not-using it.)
    url(r'^api/workflows/(?P<workflow_id>[0-9]+)', views.workflow_detail),
    url(r'^api/workflows/(?P<workflow_id>[0-9]+)/', include([
        url(r'^duplicate$', workflows.Duplicate.as_view()),
        url(r'^undo$', views.workflow_undo),
        url(r'^redo$', views.workflow_redo),

        url(r'^acl$', acl.List.as_view()),
        url(r'^acl/(?P<email>[-0-9a-zA-Z_@+.]+)$', acl.Entry.as_view()),

        url(r'^tabs/(?P<tab_id>[0-9]+)/', include([
            url(r'^addmodule$', tabs.AddModule.as_view()),
        ])),

        # WfModules (Modules applied in a workflow)
        url(r'^wfmodules/(?P<wf_module_id>[0-9]+)', views.wfmodule_detail),
        url(r'^wfmodules/(?P<wf_module_id>[0-9]+)/', include([
            url(r'^params$', views.wfmodule_params),
            url(r'^fetch$', views.wfmodule_fetch),
            url((
                r'^tiles/v(?P<delta_id>[0-9]+)/r(?P<tile_row>[0-9]+)'
                r'/c(?P<tile_column>[0-9]+).json$'
            ), views.wfmodule_tile),
            # TODO: "render" and "output" are bad names. Differentiate them.
            url(r'^render$', views.wfmodule_render),
            url(r'^output$', views.wfmodule_output),
            url(r'^embeddata$', views.wfmodule_embeddata),
            url(r'^value-counts$', views.wfmodule_value_counts),
            url(r'^dataversion/read$', views.wfmodule_dataversion_read),
            url(r'^dataversion$', views.wfmodule_dataversion),
            url(r'^notifications$', views.notifications_delete_by_wfmodule),
        ])),
    ])),
